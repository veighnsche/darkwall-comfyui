...
def test_workflow_manager_initialization(test_config):
    """Test that WorkflowManager initializes correctly."""
    mgr = WorkflowManager(test_config)
...
def test_workflow_loading_success(test_config, test_workflow_json):
    """Test successful workflow loading."""
    mgr = WorkflowManager(test_config)
    
    # Create a temporary workflow file
    workflow_file = test_config.get_config_dir() / "test_workflow.json"
    workflow_file.write_text(test_workflow_json)
    
    # Update config to use test workflow
    test_config.comfyui.workflow_path = str(workflow_file)
...
def test_workflow_loading_file_not_found(test_config):
    """Test workflow loading when file doesn't exist."""
    mgr = WorkflowManager(test_config)
    
    # Update config to non-existent file
    test_config.comfyui.workflow_path = "/nonexistent/workflow.json"
    
    with pytest.raises(WorkflowError, match="Workflow file not found"):
        mgr.load()


def test_workflow_loading_invalid_json(test_config):
    """Test workflow loading with invalid JSON."""
    mgr = WorkflowManager(test_config)
    
    # Create invalid JSON file
    workflow_file = test_config.get_config_dir() / "invalid.json"
...
def test_workflow_loading_empty_file(test_config):
    """Test workflow loading with empty file."""
    mgr = WorkflowManager(test_config)
    
    # Create empty file
    workflow_file = test_config.get_config_dir() / "empty.json"
...
def test_workflow_loading_not_a_dict(test_config):
    """Test workflow loading when JSON is not a dict."""
    mgr = WorkflowManager(test_config)
    
    # Create file with array instead of object
    workflow_file = test_config.get_config_dir() / "array.json"
...
def test_workflow_validation(test_config, test_workflow_json):
...
def test_workflow_validation_missing_prompt_node(test_config):
...
def test_workflow_validation_missing_output_node(test_config):
...
def test_workflow_caching(test_config, test_workflow_json):
...
def test_workflow_cache_invalidation(test_config, test_workflow_json):
    ...
    # Modify file
    workflow_file.write_text('{"1": {"class_type": "TestNode", "inputs": {}}}')
